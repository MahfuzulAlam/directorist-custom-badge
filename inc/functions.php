...
add_filter('directorist_custom_field_meta_key_field_args', function ($args) {
    if (get_directorist_option('enable_field_key', false)) {
        $args['type'] = 'text';
    }
    return $args;
});
...
add_filter('atbdp_caching_controls', function ($controls) {
    if (isset($controls['debugging']['fields']) && is_array($controls['debugging']['fields'])) {
        $controls['debugging']['fields'][] = 'enable_field_key';
    }
    return $controls;
});